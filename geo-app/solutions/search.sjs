...
xdmp.log("Input received by search.sjs: " + xdmp.quote(rawInput));
...
var wktStrings = cts.matchRegions(
  cts.elementReference(xs.QName("region")),
  "intersects",
  searchRegions,
  ["strings"]);
var regQuery = cts.jsonPropertyRangeQuery("region", "=", wktStrings);

var geoQueries = cts.orQuery([geoQuery, geoQueryJson, regQuery]);
